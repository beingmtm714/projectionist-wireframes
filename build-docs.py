...
DOCS = [
    ...
    ("design-directions", "Design Directions", VAULT / "Projectionist-Design-Directions.md",
     "The three options considered before choosing. A record, not a live question."),
    ...
    ("teleport", "Teleport Analysis", VAULT / "Projectionist-on-Teleport.md",
     "The architecture options, and why companion mode won."),
]

SECTIONS = [
    ("The product", ["product-spec", "sprint-plan"]),
    ("Design", ["design-language", "design-directions"]),
...
def build():
    ...
    for idx, (slug, title, path, blurb) in enumerate(DOCS):
        ...
        pager.append(f'<a href="./{prev[0]}.html">← {H.escape(prev[1])}</a>' if prev else "<span></span>")
        pager.append(f'<a href="./{nxt[0]}.html">{H.escape(nxt[1])} →</a>' if nxt else "<span></span>")

        page = f"""<header class="bar">
  <a class="back" href="./">← Documents</a>
...
<nav class="pager">{pager[0]}{pager[1]}</nav>
        ...
        (OUT / f"{slug}.html").write_text(shell(f"{title} · Projectionist", page))
    ...
    for heading, slugs in SECTIONS:
        ...
        for s in slugs:
            ...
            stamp = last_touched(path)
            rows.append(
                f'<li><a href="./{s}.html"><span class="entry">'
                f'<span class="name">{H.escape(title)}</span>'
